...
def setup_route_net():
	...
	routes = pd.read_csv('ua-routes.csv')
...
def config_graph():
	...
	for i in range(len(node_index)):
		vsz[i], vord[i], vc[i] = 5, 0, np.array([128,128,128,256*0.3])/256#np.array([128,128,128,128*0.3])/256 #light grey
...
vsz, vord, vc, fvc, esz, eord, ec = config_graph()

def show_state(cdf, entry='Cost', initial=True):
	if initial:
		output_file = 'ua-flightnet1.png'
	else:
		output_file = 'ua-flightnet2-{}.png'.format(entry)

	cdf = cdf[cdf.Airline == 'UA'].reset_index(drop=True)
...
#			vc[v],vord[v] = np.array([128,128,128,256])/256, num_node
		...
		vsz[v] = 2*(4 + bisect(intervals, ict))
	...
	highlighted = ['ABQ-DEN', 'ANC-ORD', 'ANC-SFO', 'ATL-DEN', 'ATL-EWR']
	highcolor = [[228/256, 26/256, 28/256, 1], [55/256,126/256,184/256,1],[77/256,175/256,74/256,1],[152/256,78/256,163/256,1],[255/256,127/256,0,1]]
	...
	output=output_file,output_size=(300,300))

# highlight some routes
cdf0 = pd.read_csv('ua-cost1.csv')
show_state(cdf0, initial=True)

cdf1 = pd.read_csv('ua-cost2.csv')
